...
from train.varnet import VarNet
...
def runvccspirit_varnet(subject_number, slice_number,lam=5, R=4, nacs=21, im_type='Brain', option='img'):

    # Loading model
    net = VarNet(num_cascades=12)
    if im_type == 'Brain':
        if R < 5:
            model_dir = 'train/models/Brain_T1Post_R4_varnet/Brain_T1Post_R_4.pt'
        else:
            model_dir = 'train/models/Brain_T1Post_R6_varnet/Brain_T1Post_R_6.pt'
        net.load_state_dict(torch.load(model_dir, map_location="cpu")['model_state_dict'])
    elif im_type == 'Knee':
        if R < 5:
            model_dir = 'train/models/fastMRI_mini_PD_R4_varnet/fastMRI_mini_PD_R_4.pt'
        else:
            model_dir = 'train/models/fastMRI_mini_PD_R6_varnet/fastMRI_mini_PD_R_6.pt'
        net.load_state_dict(torch.load(model_dir, map_location="cpu")['model_state_dict'])
    else:
        if R < 5:
            model_dir = 'train/models/fastMRI_mini_PD_FS_R4_varnet/fastMRI_mini_PD_FS_R_4.pt'
        else:
            model_dir = 'train/models/fastMRI_mini_PD_FS_R6_varnet/fastMRI_mini_PD_FS_R_6.pt'
        net.load_state_dict(torch.load(model_dir, map_location="cpu")['model_state_dict'])
