...
def runvccspirit(subject_number, slice_number,lam=5, R=4, nacs=21, im_type='Brain', option='img'):
    ...
    if option == 'cs':
        xsize = 320
        ksp_us_bart = np.moveaxis(X_us,0,-1)[:,:,None]
        sens_bart = np.moveaxis(sens,0,-1)[:,:,None]
        L1_ESPIRIT = bart.bart(1,'pics -R W:3:0:0.01',ksp_us_bart, sens_bart)
        im_cs = sp.resize(L1_ESPIRIT[::-1], (xsize,xsize))

        im_d = sp.resize(im_dl[::-1], (xsize,xsize))
        im_r = sp.resize(im_refine[::-1], (xsize,xsize))
        im_f = sp.resize(im_full[::-1], (xsize,xsize))
        
        return im_cs, im_d, im_r, im_f
