...
def create_argparser():
    ...
    parser = argparse.ArgumentParser(
        description='Training  MoDL Reconstruction')
...
#        im_us = transforms.complex_center_crop(im_us, tuple(im_fs.shape[2:4]))
        ...
        for j in range(3):
            cost[j+1] += metric_val[1]
        
        if epoch == 0:
            logger.info('Metric val: {}'.format(loss.item()))
...
def main(args, writer, model_name):
    ...
    save_name = '/home/kanghyun/projRSC/ESPIRIT_RSC/train/logs/train_modl_' \
...
if __name__ == "__main__":
    ...
    logger = logging.getLogger('Training-MoDLNet')
